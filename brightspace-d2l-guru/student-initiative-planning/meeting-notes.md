# Meeting notes

Newest first. Agenda formats are in [templates/meeting-agenda-template.md](templates/meeting-agenda-template.md).

---

## Today, first full group triage, all dev co-ops, no John

**Agenda**

**In flight**
- Grade calculator sandbox — building
- Code aware assignments — writing up, meets the bar for team review
- Study mode — writing up, Bhavjot's, added independently this week
- Readability assist — writing up, new this week, still unassigned

**Triage**
- Walk all four against the bar for a proposal, confirm each is actually ready for team review, not just written
- Note for the group: readability assist and study mode were both drafted as "003" in parallel on separate branches, readability assist is renumbered to 004 to fix the collision
- Any new one-liners anyone wants to add to the backlog

**Decisions needed**
- Which proposal goes to team review first, and who sets up that meeting
- Who picks up Readability assist as owner

**Actions**
- [ ] owner, what, by when

---

## Upcoming, first formal lunch with John

**Agenda**

1. **Confirm scope and sponsorship.** Walk the one pager. Want a written confirmation, even a short reply to the doc.
2. **Introductions.** Teams that own Grades and Assignments. Want two names and an offer to make the intro.
3. **Initiative visibility.** How he wants this positioned internally so it reads as sanctioned. Want his steer.

**Attach** [one-pager.md](one-pager.md)

---

## First week, Fall 2026, Nakul and John Baker

Informal. John suggested running a student focused initiative among interested co-ops with his support, additional to normal co-op work, and said to go do it.

Nothing formalised. The lunch above exists to fix that.
